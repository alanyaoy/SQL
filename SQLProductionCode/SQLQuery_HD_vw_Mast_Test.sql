...
create view [JDE_DB_Alan].[vw_Mast_Test] as
...
select ItemNumber,ShortItemNumber,Colour,StockingType,Pareto,SS_Adj,PlannerNumber,Owner_,PrimarySupplier,SupplierName,FamilyGroup_,Family_0,SellingGroup_
	    ...
	    ,case 
			when WorkCenter is not null then WorkCenter								
			when WorkCenter is null then '0'		
			end as WC 
...
from 
    ((((((( select * from 
		...
		left join ( select ItemNumber as ItemNumber_ss ,SS_Adj from JDE_DB_Alan.FCPRO_SafetyStock )  ss 
...
--  where ItemNumber in ('42.210.031')
