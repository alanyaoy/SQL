...
/****** Object:  StoredProcedure [JDE_DB_Alan].[sp_MI_2_Raw_Combine_Sim_Mismatch]    Script Date: 14/09/2018 1:17:14 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [JDE_DB_Alan].[sp_MI_2_Raw_Combine_Sim_Mismatch_1mOff] 
...
-- Insert statements for procedure here
	--SELECT <@Param1, sysname, @p1>, <@Param2, sysname, @p2>
	    ...
	    ------- Below Code use past month FC to do simulation ---- Use Aug FC --- 1 month offset FC   -- 14/9/2018  ----------------
			--- Note that you are fetching data from FC_History table, performance might suffer & might take longer time to query --- Do not use this if you have to ---

		 ------------------*** Note this Code will Output Not only Forecast but Everything **-------------------- 7/9/2018
	 
    if @Item_id is null and @dt is not  null 			
			...
			with po as (
					...
					---====================================================
					---  Need to Aggregate FC Value for Single SKU & Pick up Data only in Promo Month ---  1/8/2018
					---=====================================================
                 ,mi2fc as ( select mi2.ItemID,mi2.Date,mi2.ValidStatus,sum(mi2.Value) as value
								...
								--where mi2.ValidStatus = 'Y'	
								 --where mi2.ValidStatus in ('S','Y')				-- Need to pick up both Y and S because you might have one item appearing in multiple Project 20/8/2018				
								where mi2.ValidStatus in ('S')						-- ' S' denotes for Simulation only   --- 2/8/2018 --- only pickup S items for Simulation, when MI confirmed you need to put Y instead of S
																				   -- by pick up ONly Items with 'S' status from One(Latest) Project in Your Excel 'MI_2_Raw_Data_temp', you can add them directly on already MI-charged SKUs since last update so you will not double dip. 20/8/2018
							         and mi2.Value >0									    --- Pick up data only in Promo Month ! -- Need to pick up non Zero value  --3/8/2018
				  ...
				  --,fh as ( select * from JDE_DB_Alan.vw_FC_Hist f 
				        --     where f.myReportDate2 = cast(SUBSTRING(REPLACE(CONVERT(char(10),DATEADD(mm, DATEDIFF(m,0,GETDATE())-1,0),126),'-',''),1,6) as integer)    -- Use Last month Saved FC  -- 14/9/2018
				  ...
				  ,fc_mas as (  select fh.*,fh.myDate1 as FCDate_
                   ...
                   -- select * from fc_mas f where f.ItemNumber in ('42.210.031')
				  ...
				  -- select * from _fc where _fc.ItemNumber in ('42.210.031')
				 ...
				 -- select  * from fc  where fc.ItemNumber_f in ('42.210.031')
				 ...
				 --  select * from fc where fc.ItemNumber_f in ('709895')
					...
					,tb as 
						...
						-- where f.FcDate_f < '2019-09-02'
							   --and f.ItemNumber in ('42.210.031')
									 )
								 						
				    -- select * from tb 
					 --where tb.ItemNumber_f in ('42.210.031') 
					 --order by tb.ItemNumber_f,tb.FCDate_f
					 ...
					 ,com as ( select _t.ItemNumber_f,_t.Date_f d1,_t.Date_f_2 d3,_t.FCDate_f d2,'FC_Qty' as DataType,_t.FC_Vol as Value,_t.Stk_Out_Stauts,_t.Owner_,_t.PrimarySupplier,_t.Description,_t.FamilyGroup_,_t.Family_0,_t.Leadtime_Mth,_t.PlannerNumber from _t
								  ...
								  union all
									select _t.ItemNumber_f,_t.Date_f d1,_t.Date_f_2 d3,_t.FCDate_f d2,'End_Stk_Amt' as DataType,_t.FC_Amt as value,_t.Stk_Out_Stauts,_t.Owner_,_t.PrimarySupplier,_t.Description,_t.FamilyGroup_,_t.Family_0,_t.Leadtime_Mth,_t.PlannerNumber from _t	  
								  union all
						 ...
						 (--select * from stg as com													--- For testing purpose
						   select * from com
							...
							where --com.DataType in ('FC_Qty')
								    com.ItemNumber_f in (select distinct o.ItemID from JDE_DB_Alan.FCPRO_MI_2_Raw_Data_tmp o where o.ValidStatus in ('S')  ) 		-- Need to filter out only pick up Simulation Items --- 20/8/2018	 --- change the table name 31/7/2018	
			  ...
			  ---===============================================================================
			  -- Need to Join back Original to get Data for only MI Month you Need --- 3/8/2018						-- If you only use for Simulation, Need need for this part --- 20/8/2018
			  ---===============================================================================
			...
			--  ,fcst_ as ( select *
						...
						--  from  _fcst f left join mi2fc_ myf on  f.ItemNumber_f= myf.ItemID  and  f.d1 = myf.Date   -- wrong as it will still pick up data in non promo month
						--  from  _fcst f inner join mi2fc_ myf on  f.ItemNumber_f= myf.ItemID  and  f.d1 = myf.Date
						  --  from mi2fc_ myf inner join _fcst f on myf.ItemID = f.ItemNumber_f and myf.Date = f.d1			-- user inner join
						--)
			   ...
			   ---==========================================
		    select f.ItemNumber_f,f.d1,f.d2,f.DataType,f.value,f.Stk_Out_Stauts,f.Owner_,f.PrimarySupplier,f.Leadtime_Mth,f.PlannerNumber,f.Description
			        ,getdate() as ThisReportDate
			--select distinct f.ItemNumber_f		
			from _fcst f
			--where f.ItemNumber_f in ('42.210.031')
			order by f.ItemNumber_f,f.DataType,f.d1
    ...
    ELSE IF @Item_id is not null and @dt is not null
		 BEGIN
